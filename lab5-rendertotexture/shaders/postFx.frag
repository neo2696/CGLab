...
/*
* Select a pixel copy the texture and use it on the surrounding pixels then repeat for others
*/
void main()
{
	...
	case 6:
		...
		//fragmentColor = vec4(mosaic(gl_FragCoord.xy, textureSize(frameBufferTexture,0)),1.0);
		break;
	case 7:
...
vec2 mosaic(vec2 coords) {
	return floor(coords / filterSize) * filterSize;
}
